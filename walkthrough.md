# FleetOS — Geliştirme Walkthrough

Bu belge, FleetOS projesini **sıfır hatayla, demo/dump verisi olmadan, production-ready** hale getirmek için faz faz ilerleme planıdır.
...
| 2026-05-15 | Auth (Faz 3) | `middleware.ts` dashboard/login koruması; `DashboardAuthGuard` + cookie sync; login → `/dashboard` + `redirect` param; logout API + `clearAuth`; `pnpm --filter @fleetos/web build` ✅ |
...
| 2026-05-16 | Faz 2 | `@fleetos/shared-types`: enums, ApiResponse, entities, currency, labels; API Zod + web re-exports |
...
| 2026-05-16 | Faz 8 | `@fleetos/shared-types/rbac`; tenant-scope; RBAC + tenant tests |
...
| `@fleetos/shared-types` | ✅ API + web workspace dependency |
| Prisma migrations | ✅ `20260516120000_init` repoda; `db:migrate:deploy` |
| Testler | 🔄 API: `pnpm --filter @fleetos/api test` (10 unit tests) |
...
1. **İlk migration oluştur**
   ```bash
   pnpm --filter @fleetos/api exec prisma migrate dev --name init
   ```
...
1. **`@fleetos/shared-types` genişlet**
...
2. **API'de kullan**
   - `apps/api/package.json` → `"@fleetos/shared-types": "workspace:*"`
...
- [x] Enum duplicate yok — `@fleetos/shared-types` + Zod `nativeEnum` (API şemaları)
- [x] `pnpm --filter @fleetos/shared-types build` + api + web build yeşil
...
1. **Next.js middleware** — `apps/web/src/middleware.ts` ✅
   - `/dashboard/*` → `fleetos_access_token` cookie kontrolü
...
6. **Logout** ✅
   ...
   - `auth-cookies.ts`: localStorage + `fleetos_access_token` cookie senkronu
...
2. **API — integration (opsiyonel)** ✅
   - `health.integration.test.ts` — supertest; `FLEETOS_INTEGRATION=1` ile çalışır
...
5. **Lint** 🔄
   - API: `tsc --noEmit` (`pnpm --filter @fleetos/api lint`)

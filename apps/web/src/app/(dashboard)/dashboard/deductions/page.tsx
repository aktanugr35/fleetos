...
import { DeductionRecord } from '@/lib/deductions';
...
export default function DeductionsPage() {
  ...
  useEffect(() => {
    const load = async () => {
      setLoading(true);
      try {
        const [driversRes, deductionsRes] = await Promise.all([
          api.get('/drivers'),
          api.get('/deductions?limit=500'),
        ]);
  ...
  const summaries = useMemo(() => {
    ...
    for (const d of deductions) {
      const entry = byDriver.get(d.driver.id);
      if (!entry) continue;
      entry.totalCount += 1;
  ...
  return (
    <div>
      <PageHeader
        title="Deductions"
        description="Select a driver to view and add deductions by statement period date"
      />
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        ...
        <div className="card">
          ...
          <p className="text-2xl font-bold text-red-400 mt-1">{formatCurrency(fleetTotal)}</p>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {filtered.map(({ driver, totalCount, totalAmount }) => (
            <Link
              key={driver.id}
              href={`/dashboard/deductions/${driver.id}`}
              className="card group hover:border-blue-500/30 hover:bg-blue-500/[0.03] transition-all duration-200"
            >
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <h3 className="font-semibold text-gray-100 group-hover:text-blue-400 transition truncate">
                ...
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  className="w-5 h-5 text-gray-600 group-hover:text-blue-400 flex-shrink-0 transition"
                  viewBox="0 0 24 24"
                  fill="none"
                  stroke="currentColor"
                  strokeWidth="2"
              ...
              <div className="mt-4 pt-4 border-t border-[var(--border-color)] grid grid-cols-2 gap-3 text-center">
                ...
                <div>
                  ...
                  <p className="text-sm font-semibold text-red-400 mt-0.5">{formatCurrency(totalAmount)}</p>
                </div>
              </div>
            </Link>
          ))}
